...
source_title: "Glossary of Terms"
source: "Glossary_of_Terms.htm"
source_url: "https://downloads.languagetechnology.org/fieldworks/Documentation/en/index.htm#t=Glossary_of_Terms.htm"
source_hash: "sha256:902c7d7196f408addc2e76cc7f471f0bac6efcb24cb42bbd21d8abe7fd0faf05"
keywords:
  - "Glossary of Terms"
fw_help_version: "9.3"
type: "topic"
content_hash: "sha256:59a00cf84fa05388"
---
...
Many terms are *not* found in this glossary. You can refer to other documentation available under **Resources** on the [Help](User_Interface/Menus/Help/Help_overview.md) menu.
...
**allomorph** - (from LinguaLinks Help): An allomorph is one of two or more complementary [morphs](#M) which manifest a [morpheme](#M) in its different phonological or morphological environments. Therefore, when deciding between calling something an allomorph or a variant: Allomorphs if it is phonologically conditioned, otherwise make it a Variant for morphologically conditioned or lexically conditioned suppletion.

**Analysis Candidates** - A portion of the field label that is used has a header for wordform analyses for which the *user opinion* is set to **Unknown**. See also [Test Case Analysis](#T).

**analysis writing system** - See [writing system, analysis](#W).

**analyze** - In FieldWorks Language Explorer, the term "analyze" is understood to mean "manually parse" (words in a text). See [parse](#P) and [parser](#P), and also [gloss](#G).
...
**Bartholomew -** a dictionary configuration for bilingual dictionaries. The Bartholomew style has three distinctives: (1) focus on native speaker = simplify entry up front; (2) extensive use of illustrative sentences to discover and display meaning; (3) display of complex forms as both main entries and as subentries.

**Based on style** - See [parent style](#P).

**Best analysis writing system** - See [writing system, analysis](#W).
...
**Chorus** - Chorus is a version control system. It is the underlying code library that is used to merge the data in xml files (FieldWorks and WeSay store the data in xml files.) **See also:** [repository](#R). Chorus dialog boxes are discussed in the **Chorus Help** file.

**complex form** - a lexeme form consisting of two or more [components](#C), each of which may be a lexical entry in its own right. Examples of complex form types include compounds, derivatives, idioms, phrasal verbs and sayings.

**complex rendering** - See [rendering](#R).

**component (of complex form)** - an item that is part of a [complex form](#C). For example, *black* and *bird* are the components of the compound word *blackbird*. Similarly, *kick* and *bucket* are components of the idiom *kick the bucket*. **See also:** [subentry](#S).

**Conflicting Opinions** - a column available in **Texts & Words** which provides numerical values reflecting the number parser opinions ([predicted analyses](#P)) as compared to [user analyses](#U). **-1** means there are *no* analysis for the word at all. **0** means the parser and the user are in agreement regarding all analyses that exist. (There cannot be any analyses that are known to the parser, but that are not known to the user. If the user has any approved analyses, then the parser result is **Successful**. If the user has disapproved an analysis, then the parser will agree by *not* producing a parser failure). **1**, **2**, and so on, indicates the number of times that the parser and the user are *not* in agreement. **Tip:** Any number other than **0** (zero) in this column means there is additional work to do on that word.
...
**default writing system** - See [writing system, default](#W).
...
**ELAN -** a professional tool for the creation of complex annotations on video and audio resources. You can export interlinear data from FLEx for use in ELAN or interlinearize imported ELAN data in FLEx. **See:** <a href="https://www.philol.msu.ru/~languedoc/eng/help/flex2eaf.php" target="_blank" title="https://www.philol.msu.ru/~languedoc/eng/help/flex2eaf.php">https://www.philol.msu.ru/~languedoc/eng/help/flex2eaf.php</a>.
...
**Extensible Markup Language** - See [XML](#X).
...
**full reduplication** - see [reduplication](#R).

## G

**gloss/glossing** - (verb) The task of manually entering word glosses. See [parse](#P) and [parser](#P). See also [analyze](#A). As a noun, a gloss is a summary of the meaning of a morpheme or word, suitable for use in interlinear text displays.
...
**Keyman** - Keyman (<a href="https://keyman.com/desktop/" target="_blank" title="https://keyman.com/desktop/">https://keyman.com/desktop/</a>) is a computer program that allows you to enter text in other languages without changing your physical keyboard (or system keyboard).

## L

**layout** - a layout is the configuration for the content and display of data, such as seen in the **Dictionary** [view](Using_Tools/Lexicon_tools/Dictionary/Dictionary_overview.md).
...
**LIFT** - **L**exicon **I**nterchange **F**orma**T** is an XML format for storing lexicons/dictionaries. LIFT allows you to move that data between programs (hence the term 'interchange').

**link** - See [hyperlink](#H).
...
**main entry** - An entry in a published dictionary that alphabetizes under its headword and fully describes the headword. **See also:** [minor entry](#M) and [subentry](#S). (Refer to the main Help index for information about dictionary views.)

**marker** - A marker identifies a text element in a Standard Format file.

**menu** - A menu is a list of commands that appears when you click a menu name on a menu bar or other toolbar. **See also:** menu button under [Symbols](#Symbols).
...
**node** - In the **Configure Dictionary** dialog box, a node is a set or collection of fields. Each node has controls so you can expand (![](assets/images/User_Interface/Menus/Tools/Configure_Dictionary/Plus_box_expand.GIF)) or collapse (![](assets/images/User_Interface/Menus/Tools/Configure_Dictionary/minus_sign_box.GIF)) that set of fields. Typically, individual fields do not have these controls unless there are options such as Abbreviation or Name. The **Configure Reversal Indexes** dialog box and others are similar.
...
**partial reduplication** - see [reduplication](#R).
...
**Predicted Analyses** - A column in **Word** **Analyses** that show the number of successful parses of the wordform. In other words, the numerical value corresponds to the number of analyses in the **Wordform Analyses** pane that contains **Successful** in the **Parse result** field, regardless of the [user opinion](#U). See also [User Analyses](#U). See also: [Conflicting Opinions](#C).
...
**project** - See [FieldWorks project](#F).
...
**Requires more derivation** - See [Final](#F).

**Root** - A root is the [morpheme](#M) that is the primary or basic part of the [word](#W). Roots can sometimes occur by themselves as full words.
...
**standard rendering** - See [rendering](#R).

**stem** - A stem is a grammatical form that is part of a word. If a word can be inflected, the stem is the part left behind after the inflectional affixes are subtracted. So an inflected word is composed of a stem and one or more inflectional affixes. A stem can be a bare [root](#R), a derivation, or a compound.
...
**Test Case Analyses** - A portion of the field label that is used has a header for wordform analyses for which the user opinion is set to *Disapproved*. See also [Analysis Candidates](#A).
...
**unspecified attribute** - See [inherited attribute](#I).
...
**User Opinion** - In **Word Analyses**, you can specify your opinion of each analysis, whether approved, unknown, or disapproved. **Note:** In **Texts & Words**, you can *approve* an analysis as you choose the method of moving away from a word focus box. See also [Predicted Analysis](#P) and [Conflicting Opinions](#C).

**UTF-8 -** A variable length character encoding of the Unicode Standard that uses 8-bit sequences.

## V

**variant** - A term used in linguistics to refer to a linguistic form which is one of a set of alternatives in a given context (Crystal, 1997). Examples include spelling variants, inflectional variants and dialectal variants. In FLEx, variants are [minor entries](#M).

**vernacular writing system** - See [writing system, vernacular](#W).

## W

**wizard** - A wizard is an interactive utility within an application that guides you through each step of a particular task.

**word** - In Language Explorer, the terms 'word' and '[wordform](#W)' are used synonymously. Both refer to a orthographic word in a text. For example, in the interlinear display the first line is called the 'Word' line and contains the word. **See also**: [stem](#S), [root](#R), [wordform inventory](#W).

**wordform** - This term refers to the form of the [word](#W), irrespective of whether it has variant pronunciations or multiple meanings. Homographs share the same wordform, even though we would say they are different [lexemes](#L).

**wordform inventory** - (obsolete - now called *word list*) In Language Explorer, the list of all words used in your texts. This list is not the same as the list of [lexemes](#L) in the lexicon. While interlinearizing text you choose the lexical entry that describes the [word](#W).
...
**ZEdit -** A text editor, particularly handy for working with large SFM or XML files word processor. It was installed on your hard drive when you installed SIL FieldWorks. On the Internet, see <a href="https://software.sil.org/fieldworks/support/technical-documents/" target="_blank" title="https://software.sil.org/fieldworks/support/technical-documents/">https://software.sil.org/fieldworks/support/technical-documents/</a>.
